...
public class UndeleteObjectSpectraS3Response extends AbstractResponse
{
    ...
    public UndeleteObjectSpectraS3Response( final WebResponse response ) throws IOException
    {
        super( response );
    }
    
    
    @Override protected void processResponse() throws IOException
    {
        try
        {
            this.checkStatusCode( 200 );
            final int statusCode = this.getResponse()
                                       .getStatusCode();
            switch ( statusCode )
            {
                case 200:
                    try ( final InputStream inputStream = this.getResponse()
                                                              .getResponseStream() )
                    {
