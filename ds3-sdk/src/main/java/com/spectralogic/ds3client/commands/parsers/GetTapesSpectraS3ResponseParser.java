...
public class GetTapesSpectraS3ResponseParser extends AbstractResponseParser<GetTapesSpectraS3Response> {
    private final int[] expectedStatusCodes = new int[]{200};

    @Override
    public GetTapesSpectraS3Response parseXmlResponse(final WebResponse response) throws IOException {
        ...
        if (ResponseParserUtils.validateStatusCode(statusCode, expectedStatusCodes)) {
            ...
            case 200:
                try (final InputStream inputStream = response.getResponseStream()) {
                    final TapeList result = XmlOutput.fromXml(inputStream, TapeList.class);
                    return new GetTapesSpectraS3Response(result, pagingTotalResultCount, pagingTruncated, this.getChecksum(), this.getChecksumType());
                }
