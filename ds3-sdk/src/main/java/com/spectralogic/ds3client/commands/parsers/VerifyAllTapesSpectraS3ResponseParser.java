...
public class VerifyAllTapesSpectraS3ResponseParser extends AbstractResponseParser<VerifyAllTapesSpectraS3Response> {
    private final int[] expectedStatusCodes = new int[]{204, 207};

    @Override
    public VerifyAllTapesSpectraS3Response parseXmlResponse(final WebResponse response) throws IOException {
        final int statusCode = response.getStatusCode();
        if (ResponseParserUtils.validateStatusCode(statusCode, expectedStatusCodes)) {
            switch (statusCode) {
            case 204:
                //There is no payload associated with this code, return a null response
                return new VerifyAllTapesSpectraS3Response(null, this.getChecksum(), this.getChecksumType());
            ...
            case 207:
                try (final InputStream inputStream = response.getResponseStream()) {
                    ...
                    return new VerifyAllTapesSpectraS3Response(result, this.getChecksum(), this.getChecksumType());
                }
